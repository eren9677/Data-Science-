...
# with that we can both vialize pointed and lined data together
```
...
ggplot(data=arbuthnot, aes(x = year, y = proportion))+geom_point()+geom_line()
```
...
6. Create a new variable callue of either `TRUE` 
if that year had more boys than girls, ored `more_boys` which contains the val `FALSE` if that year did not. Based on this 
...
present <- present %>%
  mutate(more_boys = boys>girls)
ggplot(data=present,aes(x = year,y=more_boys)) + geom_boxplot()
```
