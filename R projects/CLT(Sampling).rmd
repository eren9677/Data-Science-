...
ggplot(data=samp_1,aes(x=area))+geom_histogram(binwidth=150)

samp_1 %>%
 
   summarise(mean = mean(area),meadian= median(area),max=max(area),min=min(area),sd = sd(area),iqr=IQR(area))
...
#i think the answer is 1k because more data means more insights about it always
...
sample_means_small <- ames %>%
  ...
  summarise(bar_x = mean(area))

ggplot(data = sample_means_small,aes(bar_x))+geom_histogram(binwidth=50)
```
...
#there are exactly 25 variabels in thsi sample because we replicated it 25 
```
...
#answer of this 4th question is A
