...
```{r str}
#don't forget that str in R means structure not string
str(nycflights)
```
...
```{r rdu-flights-hist}
#this chunk of code will only take flight which are headed towards RDU and assigns these rows to a new dataframe called rdu_flights
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(n = n(), inter_quartile_range = IQR(dep_delay), median = median(dep_delay), mean = mean(dep_delay) )
```
...
nycflights %>%
  group_by(month) %>%
  summarise(median_department_delay = median(dep_delay)) %>%
  arrange(desc(median_department_delay))
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_rate = sum(dep_type == "on time")/ n()) %>%
...
nycflights %>%
  group_by(dep_type) %>%
  summarise(proportion_ontime = sum(arr_type =="on time")/n(),
            proportion_delayed = sum(arr_type =="delayed")/n()
            )
